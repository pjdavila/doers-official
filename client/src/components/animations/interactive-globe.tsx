...
const InteractiveGlobe = ({ className = '' }: InteractiveGlobeProps) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      if (colorChoice < 0.5) {
        colors[i * 3] = 0.478; // Purple R
        colors[i * 3 + 1] = 0.247; // Purple G
        colors[i * 3 + 2] = 1.0; // Purple B
      } else {
        colors[i * 3] = 1.0; // Orange R
        colors[i * 3 + 1] = 0.353; // Orange G
        colors[i * 3 + 2] = 0.122; // Orange B
      }
    }
